...
export function printChain(data: any, header: string) {
...
function print(data: any, header = "") {
    ...
    for (let key in data) {
        ...
        for (let j = 0; j < arr.length; j++) {
            ...
            buffer += `
${indentation} ${indentation} [${arr[j].index}]
${indentation} ⬅️  From: ${arr[j].sender?.contractImpl?.constructor.name} ${addressEllipsis(arr[j].from)}  
${indentation} 🛄  Message: ${messageOpToName(arr[j])} ( ${coins}💎 )   
${indentation} ➡️  To: ${arr[j].contractImpl?.constructor.name} ${addressEllipsis(arr[j].contractAddress)}`.padEnd(100);
        }
        buffer += `\n`;
    }
    console.log(buffer);
