...
export class TvmBus {
    ...
    private async _broadcast(msg: InternalMessage, taskQueue: Array<Function>) {
        ...
        if (response.actionList) {
            ...
            for (let it of response.actionList) {
                ...
                console.log("actionToMessage", itMsg);
                ...
                taskQueue.push(async () => {
                    ...
                    if (it.message.init && !this.getContractByAddress(itMsg.to)) {
                        const deployedContract = (await this.deployContractFromMessage(it.message?.init?.code as Cell, it.message?.init?.data as Cell, itMsg)) as iTvmBusContract;

                        this.counters.contractDeployed++;

                        const parsedResult = parseResponse(itMsg, deployedContract.initMessageResultRaw as ExecutionResult, deployedContract, true);
                        ...
                        for (let action of parsedResult.actions) {
                            ...
                            taskQueue.push(async () => {
                                console.log(`Publishing task queue`);

                                return await this._broadcast(actionToMessage(deployedContract.address as Address, action, msg, response), taskQueue);
                            });
                        }
                        return {
    ...
    async deployContractFromMessage(codeCell: Cell, storage: Cell, message: InternalMessage) {
        console.log("deployContractFromMessage", message);
