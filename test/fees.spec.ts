...
describe("Tvm Bus on chain contracts Test Suite", () => {
    it("assert fwd fee", async () => {
        ...
        expect(firstMessage.fwdFee).to.not.null;
    ...
    it("Storage change", async () => {
        ...
        let secondMessage = messageList[2];

        expect(secondMessage.contractData?.hasChanged).to.eq(true);
        expect(secondMessage.contractData?.currentState.toString()).not.eq(
            secondMessage.contractData?.previousState.toString(),
        );
